<template>
  <section class="space-y-3">
    ...
    <!-- таблица с возможностью скрывать колонки -->
...
const { rows, meta, loading, error, fetchNow, setPage, setLimit } =
  usePaginatedEndpoint<Record<string, any>>('/incomes', () => ({
...
// ── отдельная выборка под график (НЕ зависит от пагинации таблицы)
...
// ── агрегация для графика — по chartRows
const daily = computed(() =>
  buildDailySumSeries(
    chartRows.value,
    // кандидаты под суммы для incomes (оставляем пошире, agg.ts приведёт строки к числам)
    ['sum', 'amount', 'forPay', 'totalPrice', 'payment', 'retailAmount']
  )
)
const series = computed(() => daily.value.data)
const chartTitle = computed(() =>
  daily.value.key ? `Поставки: сумма и количество по дням (${daily.value.key})` : 'Поставки: количество по дням'
)
